...
class LM(ProbMapModel):
    ...
    def log_likelihood(self):
        ...
        self.update_sum_squares()
        return -self.num_data / 2. * np.log(np.pi * 2.) - self.num_data / 2. * np.log(
            self.sigma2) - self.sum_squares / (2. * self.sigma2)
...
def relu(x, num_basis=4, data_limits=[-1., 1.], gain=None):
    ...
    Phi[:, 0] = 1.0
    for i in range(1, num_basis):
        Phi[:, i:i + 1] = (gain[i - 1] * np.asarray(x, dtype=float) > centres[i - 1]) * (
...
class BLM(LM):
    ...
    def update_QR(self):
        "Perform the QR decomposition on the basis matrix."
        self.Q, self.R = np.linalg.qr(
            np.vstack([self.Phi, np.sqrt(self.sigma2 / self.alpha) * np.eye(self.basis.number)]))
    ...
    def update_nll(self):
        ...
        self.log_det = self.num_data * np.log(self.sigma2 * np.pi * 2.) - 2 * np.log(
            np.abs(np.linalg.det(self.Q[self.y.shape[0]:, :])))
...
def mlp_cov(x, x_prime, variance=1., w=1., b=1.):
    ...
    return variance * np.arcsin(
        (w * np.dot(x, x_prime) + b) / np.sqrt((np.dot(x, x) * w + b + 1) * (np.dot(x_prime, x_prime) * w + b + 1)))
